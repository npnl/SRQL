...
function lesion_vol {
	...
	LOWER=$(echo "$MEANVAL - $STDEV" | bc -l);
	UPPER=$(echo "$MEANVAL + $STDEV" | bc -l);
	...
	if `fslmaths $ANATOMICAL -mul $SUBJECTOPDIR/Intermediate_Files/${Subject}_LESION_MASK${counter}_bin $SUBJECTOPDIR/Intermediate_Files/${Subject}_LESION${counter}_NORMRANGE` ; then
		...
		fslmaths $SUBJECTOPDIR/Intermediate_Files/${Subject}_LESION${counter}_NORMRANGE -uthr $LOWER $SUBJECTOPDIR/Intermediate_Files/${Subject}_WMAdjusted_Lesion${counter}_lower;
		
		#get rid of anything below upper bound
		fslmaths $SUBJECTOPDIR/Intermediate_Files/${Subject}_LESION${counter}_NORMRANGE -thr $UPPER $SUBJECTOPDIR/Intermediate_Files/${Subject}_WMAdjusted_Lesion${counter}_upper;
		
		#sum two together
		fslmaths $SUBJECTOPDIR/Intermediate_Files/${Subject}_WMAdjusted_Lesion${counter}_upper -add $SUBJECTOPDIR/Intermediate_Files/${Subject}_WMAdjusted_Lesion${counter}_lower $SUBJECTOPDIR/Intermediate_Files/${Subject}_WMAdjusted_Lesion${counter};
...
function create_subj_array () {
	...
	for LESION in $LESIONFILES; do
			...
			if [ "$SubjLesionVol" != 'X' ]; then
				...
				PercentVol=$(awk "BEGIN {printf \"%.9f\",${SubjLesionVol}/${TotalNativeBrainVol}}");
...
echo "    SRQL Toolbox  Copyright (C) 2017  Kaori L Ito & Sook-Lei Liew
